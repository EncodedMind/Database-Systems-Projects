...
void schema_init(TableSchema *schema, const AttributeSchema *attrs,
                 const int attribute_count, const char *key_attr_name) {
    schema->count =
        attribute_count < MAX_ATTRIBUTES ? attribute_count : MAX_ATTRIBUTES;
    schema->key_index = -1;
...
void record_create(const TableSchema *schema, Record *record, ...) {
    ...
    for (int i = 0; i < schema->count; i++) {
        ...
        case TYPE_CHAR: {
            ...
            record->values[i].string_value[attr->length - 1] = '\0';
            break;
        }
        default:
            break;
...
DataType record_get_value(const TableSchema *schema, const Record *record,
                          const char *attr_name, char *output) {
    for (int i = 0; i < schema->count; i++) {
        if (strcmp(schema->attributes[i].name, attr_name) == 0) {
            const AttributeSchema *attr = &schema->attributes[i];
