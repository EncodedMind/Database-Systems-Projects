...
int get_random_number(const int max) { return rand() % max; }

TableSchema employee_get_schema() {
    const AttributeSchema attrs[] = {{"id", TYPE_INT, 0},
                                     {"name", TYPE_CHAR, 20},
...
void employee_random_record(const TableSchema *schema, Record *record) {
    ...
    record_create(schema, record, id, name, surname, city);
}
...
void student_random_record(const TableSchema *schema, Record *record) {
    ...
    record_create(schema, record, id, name, surname, university, department);
}
